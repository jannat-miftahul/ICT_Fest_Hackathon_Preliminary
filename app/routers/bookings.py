...
def _refund_amount(price_cents: int, refund_percent: int) -> int:
    amount = (Decimal(price_cents) * Decimal(refund_percent) / Decimal(100)).quantize(
...
def create_booking(
    ...
    start = _parse_booking_datetime(payload.start_time)
    end = _parse_booking_datetime(payload.end_time)
    now = datetime.utcnow()
...
def cancel_booking(
    ...
    with _booking_lock:
        ...
        refund_amount_cents = _refund_amount(booking.price_cents, refund_percent)
